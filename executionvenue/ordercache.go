...
func NewOrderCache(store orderstore.OrderStore, loadOrder func(order *model.Order) bool) (*OrderCache, error) {
	orderCache := OrderCache{
		store: store,
	}

	var err error
	orderCache.cache, err = store.RecoverInitialCache(loadOrder)
	if err != nil {
		return nil, err
	}
